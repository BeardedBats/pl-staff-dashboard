...
- Added five committed visual-regression baselines: anonymous mobile login, administrator desktop home, responsive mobile Archive, the shared template-dialog control surface, and EIC desktop Analytics. Animations, caret, reduced motion, color scheme, time, viewport, data actors, and snapshot paths are deterministic; a 3% cross-platform pixel allowance retains meaningful layout/style detection without treating Windows/Linux font rasterization as product drift.
