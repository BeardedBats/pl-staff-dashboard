...
| P1.6 | Shared JSON/query parsing, stable errors, bounded client messages, and no raw upstream/database leakage | `src/lib/api/http.test.ts`; `src/lib/api/client.test.ts`; `src/lib/api/route-contracts.integration.test.ts` |
...
| P1.14 | Patched dependency graph with no dormant runtime packages | Fresh `npm ci`, `npm audit`, and the recorded unused-dependency scan; test architecture landed in P2.1 and CI enforcement remains P2.9 |
